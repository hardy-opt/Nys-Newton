...
function [w,infos] = svrg1(problem,options)
...
f_valn=f_val+1;
    while (t<=T) %|| f_val - f_valn>10^-16
    %while f_valn - f_val>10^-16
    %if mod(t,K)==0
    w0=w;
    ...
    options.n = n;
                ...
                if strcmp(options.method,'avg')
                    
                g_new = ((i-1)/i)*g_new+(1/i)*full_grad;
                %b = 0.9;
                
                
                g = g_new +problem.grad(w,ind) - problem.grad(w0,ind);
