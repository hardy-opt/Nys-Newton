...
function [w,infos] = svrg_abs_dh(problem,options)
...
start_time = tic();
    ...
    if options.verbose > 0
       fprintf('SVRG ABS DH: Epoch = %03d, cost = %.5e, elapsed_time = %.4e, gnorm=%.5e\n', epoch, f_val, 0, gnorm);
    end  
    ...
    grad_calc_count = grad_calc_count + n;
     ...
     options.n = n;
            ...
            if epoch ==  0
                ...
                bb = A.*vec -Atk.*vec;
                norm(bb);
                g = grad_w - grad_0 + full_grad +(1)*bb;
        ...
        % display infos
        if options.verbose > 0
            fprintf('SVRG ABS DH: Epoch = %03d, cost = %.5e,time = %.4e, ||bb||=%.5e, ||g||=%.5e, optgap = %.5e\n', epoch, f_val, elapsed_time,norm(bb), gnorm, optgap);
        end
