...
function [w,infos] = svrg_app(problem,options)
...
start_time = tic();
    ...
    if options.verbose > 0
       fprintf('SVRG App: Epoch = %03d, cost = %.5e,time = %.4e, ||g||=%.5e\n', epoch, f_val, 0, gnorm);
    end  
...
%                     
                    fprintf('Size of G = %d\n',length(G));
                    [aha,fn1,apta,orta] = problem.app_hess(w0,1:n,set);
...
%                 %g = problem.grad(w,ind) - problem.grad(w0,ind) + full_grad + problem.hess_vec() + problem.hess_vec();
        ...
        % display infos
        if options.verbos > 0
            fprintf('SVRG App: Epoch = %03d, cost = %.5e, time = %.4e, ||bb|| =%.5e, ||g|| =%.5e, optgap = %.5e\n', epoch, f_val, elapsed_time,norm(bb), gnorm, optgap);
        end
    ...
    figure;
    semilogy(1:T,NONE(1:T),LT{1},1:T,NTWO(1:T),LT{2},1:T,NTHREE(1:T),LT{3},1:T,NFOUR(1:T),LT{4},'MarkerSize', markersize, 'Linewidth', 4); hold on;
    %axis auto
    xlabel('  Epoch ','FontSize',30);
    ylabel(' ||H - A||_F  with k ','FontSize',30);
    lno = legend('k=10','k=7','k=5','k=3');
    lno.FontSize = 25;
    ...
    figure; semilogy(1:T+1,APTA(1:T+1),LT{1},1:T+1,APTB(1:T+1),LT{2},1:T+1,APTC(1:T+1),LT{3},1:T+1,APTD(1:T+1),LT{4},1:T+1,OTA(1:T+1),LT{5},'MarkerSize', markersize, 'Linewidth', 4); 
    axis auto
    xlabel('  Epoch ','FontSize',30);
    ylabel('  Time: original Hess. & Approx. Hess(sec) ','FontSize',30);
    lnt = legend('k=10 Approx','k=7 Approx','k=5 Approx','k=3 Approx','Orig Hess');
    lnt.FontSize = 25;
    ax = gca;
    ax.XAxis.FontSize = 20;
    ax.YAxis.FontSize = 20;
