...
function [w,infos] = svrg_dh(problem,options)
...
start_time = tic();
    ...
    if options.verbose > 0
       fprintf('SVRG DH: Epoch = %03d, cost = %.5e, elapsed_time = %.4e, gnorm=%.5e\n', epoch, f_val, 0, gnorm);
    end  
...
%                 
%                 g0= problem.grad(w0,ind);
%                 grad = problem.grad(w,ind);
%                 
%                 
%                 % update step-size
%                 g = grad - g0 + full_grad;
...
%         
        ...
        % display infos
        if options.verbos > 0
            fprintf('SVRG DH: Epoch = %03d, cost = %.5e,time = %.4e, ||bb||=%.5e, ||g||=%.5e, optgap = %.5e\n', epoch, f_val, elapsed_time,norm(bb), gnorm, optgap);
        end
